...
export function BillingActions({
  ...
  return (
    <div className="mt-6 flex flex-wrap gap-3">
      {canResubscribe ? (
        <Link href={`/assinar?plano=${encodeURIComponent(planId)}`} className="inline-flex h-11 items-center rounded-xl bg-cyan-700 px-5 text-sm font-semibold text-white hover:bg-cyan-800">
          Assinar novamente
        </Link>
      ) : null}
      <button onClick={() => run("refresh")} disabled={loading !== null} className={`h-11 rounded-xl px-5 text-sm font-semibold disabled:opacity-60 ${canResubscribe ? "border border-slate-300 bg-white text-slate-700" : "bg-slate-900 text-white"}`}>{loading === "refresh" ? "Atualizando..." : "Atualizar status"}</button>

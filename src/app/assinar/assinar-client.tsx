        ...
        <div className="mt-5 rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm sm:p-9">
          ...
          {method === "pix" ? <div className="mt-6 rounded-2xl border border-emerald-200 bg-emerald-50/60 p-5">
            {!pixAvailable ? <p className="text-sm font-semibold text-amber-800">{testMode ? "Pix manual desativado no modo de teste." : "Pix manual temporariamente indisponível. Configure os dados de recebimento na Vercel."}</p> : <>
              <label className="text-sm font-semibold text-slate-900" htmlFor="customer-name">Nome para identificação do comprovante</label><input id="customer-name" value={name} onChange={(event) => setName(event.target.value)} className="mt-2 h-12 w-full rounded-xl border border-emerald-200 bg-white px-4 text-sm outline-none" placeholder="Seu nome" />
              {pixStatus === "approved" ? <div className="mt-4 rounded-xl bg-emerald-100 p-4 text-sm font-semibold text-emerald-900">Acesso liberado. Seu Pix foi aprovado.</div> : pixStatus === "pending" ? <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 p-4"><p className="font-semibold text-amber-900">Aguardando confirmação</p><p className="mt-1 text-sm text-amber-800">Pague via Pix e envie o comprovante para ativação.</p></div> : pixStatus === "rejected" ? <div className="mt-4 rounded-xl border border-rose-200 bg-rose-50 p-4 text-sm font-semibold text-rose-800">Comprovante recusado. Confira os dados e crie um novo pedido.</div> : null}
              <div className="mt-4 rounded-xl border border-emerald-200 bg-white p-4 text-sm text-slate-700"><p><strong>Plano:</strong> {pixPlan.name}</p><p className="mt-2"><strong>Valor:</strong> R$ {pixPlan.price}</p><p className="mt-2"><strong>Recebedor:</strong> {pixConfig.receiverName}</p>{pixConfig.receiverDocument ? <p className="mt-2"><strong>Documento:</strong> {pixConfig.receiverDocument}</p> : null}<div className="mt-3 flex items-center gap-2 rounded-lg bg-slate-100 p-3"><code className="min-w-0 flex-1 break-all text-xs">{pixConfig.key}</code><button type="button" onClick={() => void navigator.clipboard.writeText(pixConfig.key)} className="rounded-lg border bg-white p-2" title="Copiar chave Pix"><Copy className="h-4 w-4" /></button></div></div>
              {pixStatus !== "pending" && pixStatus !== "approved" ? <button type="button" onClick={createPixOrder} disabled={loading !== null} className="mt-5 flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-emerald-700 px-6 text-sm font-semibold text-white hover:bg-emerald-800 disabled:opacity-60"><QrCode className="h-5 w-5" />{loading === "pix" ? "Criando pedido..." : "Pagar por Pix manual"}</button> : null}
              {pixStatus === "pending" ? <a href={whatsappUrl} target="_blank" rel="noopener noreferrer" className="mt-4 flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-emerald-700 px-5 text-sm font-semibold text-white"><MessageCircle className="h-5 w-5" />Enviar comprovante pelo WhatsApp</a> : null}
            </>}
          </div> : null}

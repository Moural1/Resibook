...
import { AlertTriangle, ArrowLeft, Check, CreditCard, ShieldCheck } from "lucide-react";
import { BILLING_PLANS, getBillingPlan, type BillingPlanId } from "@/lib/billing/plans";

function AssinarContent({ testMode }: { testMode: boolean }) {
  ...
  async function checkout() {
    ...
    try {
      ...
      setError(
        checkoutError instanceof Error ? checkoutError.message : "Pagamento indisponível."
      );
      setLoading(false);
    }
  }
  ...
  return (
    <main className="min-h-screen bg-slate-100 px-4 py-10">
      <div className="mx-auto max-w-4xl">
        <Link href="/dashboard" className="inline-flex items-center gap-2 text-sm font-semibold text-slate-600">
          <ArrowLeft className="h-4 w-4" />Voltar
        </Link>
        <div className="mt-5 rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm sm:p-9">
          {testMode ? (
            <div className="mb-6 flex items-start gap-3 rounded-2xl border border-amber-300 bg-amber-50 p-4 text-amber-950">
              <AlertTriangle className="mt-0.5 h-5 w-5 shrink-0" />
              <div>
                <p className="font-semibold">Modo de teste do Mercado Pago</p>
                <p className="mt-1 text-sm">Use somente comprador e cartão de teste. Esta assinatura não libera nem bloqueia acessos reais.</p>
              </div>
            </div>
          ) : null}
          ...
          <button type="button" onClick={checkout} disabled={loading} className="mt-6 flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-cyan-700 px-6 text-sm font-semibold text-white hover:bg-cyan-800 disabled:opacity-60">
            <CreditCard className="h-5 w-5" />{loading ? "Abrindo pagamento..." : `${testMode ? "Testar" : "Assinar"} plano ${BILLING_PLANS[selected].name}`}
          </button>
...
export function AssinarClient({ testMode }: { testMode: boolean }) {
  return <Suspense fallback={<div className="min-h-screen bg-slate-100" />}><AssinarContent testMode={testMode} /></Suspense>;
}

...
export default function FlashcardsBrowser({ flashcards }: Props) {
  ...
  const filteredFlashcards = useMemo(() => {
    return rankSearchResults(flashcards, query, (card) => [
      { value: card.frente, weight: 10 },
      { value: card.tags?.join(" "), weight: 6 },
      { value: card.source_group, weight: 4 },
      { value: card.verso, weight: 2 },
      { value: card.source_file, weight: 1 },
    ]);
  }, [flashcards, query]);
